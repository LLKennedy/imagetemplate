...
func ParseColourStrings(colours ColourStrings, prefix string, inputProps map[string][]string) (color.NRGBA, map[string][]string, error) {
	colour := color.NRGBA{}
	var err error
	props, newVal, parseErr := render.ExtractSingleProp(colours.R, prefix+"R", render.Uint8Type, inputProps)
	if parseErr != nil {
		err = CombineErrors(err, parseErr)
	} else if newVal != nil {
		colour.R = newVal.(uint8)
	}
	props, newVal, parseErr = render.ExtractSingleProp(colours.G, prefix+"G", render.Uint8Type, props)
	if parseErr != nil {
		err = CombineErrors(err, parseErr)
	} else if newVal != nil {
		colour.G = newVal.(uint8)
	}
	props, newVal, parseErr = render.ExtractSingleProp(colours.B, prefix+"B", render.Uint8Type, props)
	if parseErr != nil {
		err = CombineErrors(err, parseErr)
	} else if newVal != nil {
		colour.B = newVal.(uint8)
	}
	props, newVal, parseErr = render.ExtractSingleProp(colours.A, prefix+"A", render.Uint8Type, props)
	if parseErr != nil {
		err = CombineErrors(err, parseErr)
	} else if newVal != nil {
