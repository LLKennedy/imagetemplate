...
func TestLoadComponentsFile(t *testing.T) {
	...
	t.Run("no file error", func(t *testing.T) {
		builder.fs = func() vfs.FileSystem {
			reader := fs.NewMockFileSystem()
			reader.On("Open", "myfile.json").Return(fs.NewMockFile("", []byte("hello")), nil)
			return reader
		}()
		newBuilder, err := builder.LoadComponentsFile("myfile.json")
		assert.Equal(t, builder, newBuilder)
...
func TestSetBackgroundImageData(t *testing.T) {
	...
	testFunc := func(test testSet, t *testing.T) {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.result.Canvas, result.Canvas)
			assert.Equal(t, test.result.Components, result.Components)
			assert.Equal(t, test.result.NamedProperties, result.NamedProperties)
			if test.err == nil {
				assert.NoError(t, err)
			} else {
	...
	tests := []testSet{
		...
		{
			name:     "valid b64",
			template: Template{BaseImage: BaseImage{Data: ""}},
			result:   ImageBuilder{}.SetCanvas(render.ImageCanvas{}.SetPPI(72).SetUnderlyingImage(&image.NRGBA{Pix: []uint8{0x87, 0x00, 0x15, 0xff}, Rect: image.Rect(0, 0, 1, 1), Stride: 4})).(ImageBuilder),
		},
		{
			name: "valid jpeg",
			builder: ImageBuilder{
		...
		{
			name: "file error",
			builder: ImageBuilder{fs: func() vfs.FileSystem {
				reader := fs.NewMockFileSystem()
				reader.On("Open", "somefile.jpg").Return(fs.NilFile, fmt.Errorf("some file error"))
				return reader
			}()},
			template: Template{BaseImage: BaseImage{FileName: "somefile.jpg"}},
			result: ImageBuilder{fs: func() vfs.FileSystem {
				reader := fs.NewMockFileSystem()
				reader.On("Open", "somefile.jpg").Return(fs.NilFile, fmt.Errorf("some file error"))
				return reader
			}()},
		...
		},
		{
			...
			builder: ImageBuilder{fs: func() vfs.FileSystem {
				reader := fs.NewMockFileSystem()
				reader.On("Open", "somefile.jpg").Return(fs.NilFile, nil)
				return reader
			}()},
			template: Template{BaseImage: BaseImage{FileName: "somefile.jpg"}},
			result: ImageBuilder{fs: func() vfs.FileSystem {
